...
def generate_cluster_dataset(N, n_samples_total, seed):
    """
    Generate dataset by sampling (J1, J2) uniformly from [-4, 4]^2.
    Labels determined by phase boundaries |J1|=1, |J2|=1:
      0 : |J1| < 1, |J2| < 1  (trivial)
      1 : |J1| > 1, |J2| < 1  (ferromagnetic)
      2 : |J1| < 1, |J2| > 1  (SPT)
      3 : |J1| > 1, |J2| > 1  (anti-ferromagnetic)
    """
    np.random.seed(seed)
    data, labels = [], []

    for _ in range(n_samples_total):
        J1 = np.random.uniform(-4, 4)
        J2 = np.random.uniform(-4, 4)

        if abs(J1) < 1 and abs(J2) < 1:
            label = 0
        elif abs(J1) > 1 and abs(J2) < 1:
            label = 1
        elif abs(J1) < 1 and abs(J2) > 1:
            label = 2
        else:
    ...
    n_per_phase = np.array([np.sum(labels == i) for i in range(4)])
...
while contador < 500:
    ...
    for it in range(epochs):
        for Xbatch, ybatch in iterate_minibatches(X_train, y_train, batch_size):
            def cost_fn(pc, pp, pf1, pf2, pf3):
                return cost(pc, pp, pf1, pf2, pf3, Xbatch, ybatch, reg, dm_labels)
            params_convolutions, params_poolings, params_F1, params_F2, params_F3 = opt.step(
                cost_fn, params_convolutions, params_poolings, params_F1, params_F2, params_F3
            )
    ...
    # Discard points with insufficient training accuracy
